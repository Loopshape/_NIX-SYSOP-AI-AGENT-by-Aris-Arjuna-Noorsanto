#!/usr/bin/env bash
#
# SysOp-AI: A Self-Contained, Self-Optimizing AI Command-Line Application
#
# This script provides a framework for interacting with a pool of local language
# models. It dynamically selects the best models for a given task based on
# their historical performance stored in its SQLite database.
#
# USAGE:
...
# -------------------- Models (Now a Default/Fallback) --------------------
declare -A MODEL_WEIGHTS
DEFAULT_POOL_MODELS=("2244:latest" "core:latest" "loop:latest" "coin:latest" "code:latest")
...
# -------------------- Enhanced Logging & Thinking (## HARDENED ##) --------------------
log_event() {
...
show_reasoning() {
    ...
    if [ "$SHOW_REASONING" = "true" ] && [ -n "$reasoning" ]; then
        ...
        sqlite3 "$AI_DB" <<EOF || true
INSERT INTO reasoning_chains (task_id, step, reasoning, context) VALUES ('$PROOF_TASK_ID', $PROOF_CYCLE_INDEX, '$escaped_reasoning', '$escaped_context');
EOF
    fi
}
...
# -------------------- Helpers (Updated & ## HARDENED ##) --------------------
add_memory_enhanced() {
    local prompt_esc=$(sed "s/'/''/g" <<< "$1") response_esc=$(sed "s/'/''/g" <<< "$2")
    local proof_state_esc=$(sed "s/'/''/g" <<< "$3") framework_esc=$(sed "s/'/''/g" <<< "$4")
    local complexity_val="$5" reasoning_log_esc=$(sed "s/'/''/g" <<< "$6")
    sqlite3 "$AI_DB" <<EOF
INSERT INTO memories (prompt, response, task_id, proof_state, framework, complexity, reasoning_log)
VALUES (
  '$prompt_esc', '[XOR$(printf "%d" "0x$(echo -n "$2" | sha256sum | cut -c1-8)")] $response_esc',
  '$PROOF_TASK_ID', '$proof_state_esc', '$framework_esc', $complexity_val, '$reasoning_log_esc'
);
EOF
}

add_blob() {
    local filename_esc=$(sed "s/'/''/g" <<< "$1")
    local content_esc=$(sed "s/'/''/g" <<< "$2")
    sqlite3 "$BLOBS_DB" <<EOF
INSERT INTO blobs (filename, content) VALUES ('$filename_esc', '$content_esc');
EOF
}
...
# -------------------- Dynamic Model Selection (## HARDENED ##) --------------------
_select_dynamic_models() {
    local framework="$1" complexity="$2" POOL_SIZE=3
    thinking "Selecting dynamic model pool for task..." 1
    local available_models
    mapfile -t available_models < <("$OLLAMA_BIN" list | awk 'NR>1 {print $1}')
    if [[ ${#available_models[@]} -eq 0 ]]; then
        log_event "ERROR" "No Ollama models found. Please run 'ollama pull <model>'."
        POOL_MODELS=("${DEFAULT_POOL_MODELS[@]}")
        return
    fi
    declare -A model_scores
    local reasoning_text="Scoring available models based on historical performance:\n"
    for model in "${available_models[@]}"; do
        local query="
            ...
            WHERE T2.model_name = '${model}' AND T1.framework LIKE '%${framework}%';
        "
        local score
        # Use a "here string" to pipe the query, avoiding argument length limits.
        score=$(sqlite3 "$AI_DB" <<< "$query" 2>/dev/null)
        model_scores["$model"]=${score:-0}
        reasoning_text+="  - Model: ${model}, Score: ${model_scores[$model]}\n"
    done
    local sorted_models_str
    sorted_models_str=$(for model in "${!model_scores[@]}"; do
        printf "%s %s\n" "${model_scores[$model]}" "$model"
    done | sort -rn | head -n "$POOL_SIZE" | awk '{print $2}')
    mapfile -t POOL_MODELS < <(echo "$sorted_models_str")
    if [[ ${#POOL_MODELS[@]} -lt $POOL_SIZE ]] && [[ ${#POOL_MODELS[@]} -lt ${#available_models[@]} ]]; then
        reasoning_text+="\nNot enough historical data; filling pool with other available models.\n"
        for model in "${available_models[@]}"; do
            if [[ ! " ${POOL_MODELS[*]} " =~ " ${model} " ]]; then
                POOL_MODELS+=("$model")
                if [[ ${#POOL_MODELS[@]} -ge $POOL_SIZE ]]; then break; fi
            fi
        done
    fi
    reasoning_text+="\nSelected dynamic pool for this run: ${POOL_MODELS[*]}"
    show_reasoning "$reasoning_text" "Dynamic Model Selection"
}

_update_model_usage() {
    thinking "Logging model usage for this task..." 2
    for model in "${POOL_MODELS[@]}"; do
        sqlite3 "$AI_DB" "INSERT OR IGNORE INTO model_usage (task_id, model_name) VALUES ('$PROOF_TASK_ID', '$model');"
    done
}
...
run_ai_loop() {
    ...
    _select_dynamic_models "$PROOF_FRAMEWORKS" "$PROOF_COMPLEXITY"
    ...
    echo -e "${COLOR_GRAY}Complexity: $PROOF_COMPLEXITY | Target: $PROOF_FRAMEWORKS | Dynamic Pool: ${POOL_MODELS[*]}$COLOR_RESET\n"
...
_show_main_help() {
    printf "${COLOR_BRIGHT}SysOp-AI: A Self-Contained, Self-Optimizing AI CLI${COLOR_RESET}\n\n"
    printf "This script uses a mathematical scoring system to dynamically select the best AI\n"
    printf "models for a given task based on their historical performance.\n\n"
...
_show_run_help() {
    ...
    printf "Runs the main AI loop. It will first analyze the prompt, then dynamically select\n"
    printf "the best-performing models from its history before starting the task.\n\n"
    ...
    printf "  ${COLOR_GREEN}--import <url>${COLOR_RESET}\t Import a URL's content into the context before running.\n"
...
_cmd_status() {
    ...
    printf "  Default Models Pool:\n"
    for model in "${DEFAULT_POOL_MODELS[@]}"; do

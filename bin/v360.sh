#!/usr/bin/env bash
# mega_ai_cli.sh - Bulletproof Unified AI & DevOps CLI v360+
# Recursive parser, code fixer, file hasher, DevOps assistant
# Mandatory token streaming
...
run_model_streaming() {
    ...
    stdbuf -o0 "$OLLAMA_BIN" run "$model" "$prompt" | tee /dev/tty
}
...
extract_refs_from_file() { local f="$1"; grep -Eo '(src|href|include|require|import)["\']?[^"'" ]+["\']?' "$f" 2>/dev/null | sed -E 's/.*["\']([^"\']+)["\']?.*/\1/' || true; }
follow_refs() {
    local start="$1" visited="$(mktemp)"; trap 'rm -f "$visited"' EXIT
    find "$start" -type f -print0 | while IFS= read -r -d '' f; do echo "$f" >> "$visited"; done
    local idx=1
    while true; do
        local total=$(wc -l < "$visited"); [[ $idx -gt $total ]] && break
        local current=$(sed -n "${idx}p" "$visited"); idx=$((idx+1))
        while IFS= read -r ref; do
            local absref=$(resolve_ref_to_abs "$(dirname "$current")" "$ref")
            [[ -f "$absref" ]] && grep -qxF "$absref" "$visited" || echo "$absref" >> "$visited"
...
run_unified_agent() {
    ...
    local project_dir="$PROJECTS_DIR/$task_id"; mkdir -p "$project_dir"; cd "$project_dir"
    log_success "Workspace: $project_dir"

    log_info "Hashing and analyzing files..."
    local files=$(follow_refs ".")
    hash_files_verbose "."

    for f in $files; do
        log_info "Analyzing file: $f"
        local code=$(cat "$f")
        local ai_response=$(run_model_streaming "$AGENT_MODEL" "Analyze and suggest fixes step-by-step. File: $code")
        local fixed_code=$(echo "$ai_response" | awk '/\[FIX\]/{f=1;next}/\[\/FIX\]/{f=0} f')
        if [[ -n "$fixed_code" ]]; then
            echo -e "\nDiff for $f:"; diff --color=always <(echo "$code") <(echo "$fixed_code") || true
            read -p "Apply changes? [y/N]: " c
            [[ "$c" =~ ^[Yy]$ ]] && { echo "$fixed_code" > "$f"; hash_file "$f"; }
        fi
    done

    local final_answer=$(run_model_streaming "$AGENT_MODEL" "Summarize all analyzed files and required fixes.")
    add_memory "agent" "$user_prompt" "$final_answer" "SUCCESS" "$task_id"
    log_success "Agent task complete"
    echo -e "\033[32mFINAL OUTPUT:\033[0m\n$final_answer"
}

# --- HELP ---
show_help() { echo -e "${GREEN}Mega AI CLI v360+SQL\nUsage: mega_ai_cli.sh [prompt]${NC}"; }

...
util_hash() {
	if [ -z "$1" ]; then
		echo "Usage: ai hash <string|file>"
		return 1
	fi
	if [ -f "$1" ]; then
		sha256sum "$1"
...
util_download() {
	if [ -z "$1" ]; then
		echo "Usage: ai download <url> [output_filename]"
		return 1
	fi
	local url="$1"
...
util_scan() {
	if [ -z "$1" ]; then
		echo "Usage: ai scan <pattern> [directory]"
		return 1
	fi
	local pattern="$1"
	local dir="${2:-.}"
...
util_btc() {
	util_wallet_init
	if [ -z "$1" ] || [ -z "$2" ]; then
		echo "Usage: ai btc <buy|sell> <amount_usd>"
		return 1
	fi
	source "$WALLET_FILE"
	local action="$1"

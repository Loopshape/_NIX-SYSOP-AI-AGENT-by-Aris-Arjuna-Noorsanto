...
# Version: 8.3.3 (Robust CLI Argument Parsing & Concurrency Fixes)
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            --file)
                ...
                file_context="$2"
                orchestrator_args+=("--file=$file_context")
                shift 2
                ;;
            --project)
...
main() {
    ...
    log_event "DEBUG" "Script started: WebDev Code-Engine v8.3.3"
    ...
    local prompt_args=()
    local file_context_for_orchestrator=""
    ...
    while [[ $# -gt 0 ]]; do
        case "$1" in
            ...
            --hash)
                ...
                ;;
            repair)
                if [[ -z "${2:-}" ]]; then log_event "ERROR" "Usage: webdev-ai repair <file_path>"; return 1; fi
                tool_code_repair "$2"
                command_found=true
                shift 2
                ;;
            status)
                ...
                shift
                ;;
            --file) # This is a context file for the AI, not a main command
                if [[ -z "${2:-}" ]]; then log_event "ERROR" "Error: --file requires a path."; return 1; fi
                file_context_for_orchestrator="$2"
                shift 2
                ;;
            --help|-h)
                show_help
    ...
    if ! $command_found && [ ${#prompt_args[@]} -gt 0 ]; then
        ...
        local task_args=()
        if [[ -n "$file_context_for_orchestrator" ]]; then
            task_args+=("--file=$file_context_for_orchestrator")
        fi
        # If a project was specified via --start, it's already handled by the session file.
        # The run_webdev_task function will pick it up from SESSION_FILE if no --project is explicitly passed to it.
        run_webdev_task "$full_prompt" "${task_args[@]}"
...
show_help() {
    cat << EOF
${COLOR_GREEN}WebDev Code-Engine v8.3.3 - Unified Hybrid Agent${COLOR_RESET}
...
${COLOR_CYAN}Usage:${COLOR_RESET}
  ...
  webdev-ai --file <path>                  - Provide a file as context for the AI task
...
${COLOR_CYAN}Examples:${COLOR_RESET}
  ...
  webdev-ai --file ./src/existing_code.js "Refactor this code to use async/await."
